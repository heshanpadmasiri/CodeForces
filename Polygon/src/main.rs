...
fn solve(input: &mut Reader) -> Vec<String> {
    ...
    (0..t).map(|_| {solve_polygon(input)}).collect()
}

fn solve_polygon(input: &mut Reader) -> String {
    let n = read_int(input);
    let mut polygon = read_polygon(input, n);
    // mark border hits as valid
    for i in 0..n {
        if polygon[n-1][i] == State::PossibleHit {
            polygon[n-1][i] = State::Hit;
        }
        if polygon[i][n-1] == State::PossibleHit {
            polygon[i][n-1] = State::Hit;
        }
    }
    for x in 0..n-1 {
        for y in 0..n-1 {
            let i = (n-2)-x;
            let j = (n-2)-y;
            if polygon[i][j] == State::PossibleHit {
                if polygon[i][j+1] == State::Hit || polygon[i+1][j] == State::Hit {
                    polygon[i][j] = State::Hit;
                }
                else {
                    return "NO".to_owned();
                }
...
type Polygon = Vec<Vec<State>>;

fn read_polygon(input: &mut Reader, n: Int) -> Polygon {
    (0..n).map(|_| { parse_line(read_line(input)) }).collect()
}
