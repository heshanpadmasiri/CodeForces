...
fn solve(input: &mut Reader) -> Vec<String> {
    ...
    let mut counts = HashMap::<Int, Int>::new();
    let mut min = Int::MAX;
    let mut max = Int::MIN;
    for each in nums {
        match counts.get(&each) {
            Some(count) => {
                counts.insert(each, count + 1);
            }
            None => {
                counts.insert(each, 1);
            }
        }
        if each < min {
            min = each;
    ...
    let n_ways = if max != min {
        counts.get(&min).expect("expect min to exists")
            * counts.get(&max).expect("expect max to exist")
    } else {
        (n * (n - 1)) / 2
    };
    [format!("{} {}", max_diff, n_ways)].to_vec()
}
